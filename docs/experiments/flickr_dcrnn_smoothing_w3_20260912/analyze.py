...
def report():
    ...
    for arm in LABELS:
        root = OUT / 'raw' / f'dcrnn_{arm}_s42'
    ...
    fig.suptitle('Flickr | DCRNN | W=3 | seed 42' + ('' if complete else ' | IN PROGRESS'))
    for suffix in ('png', 'pdf'):
        fig.savefig(OUT / f'convergence.{suffix}', dpi=170)
    plt.close(fig)
    lines = ['# Flickr W=3 DCRNN', '', '**状态：'+('三组已完成。' if complete else '运行中，当前曲线不是最终收敛结论。')+'**', '',
...
if __name__ == '__main__':
    with (OUT / '.plot.lock').open('w') as lock:
        fcntl.flock(lock, fcntl.LOCK_EX)
        report()
        subprocess.run(['/home/zlj/.miniconda3/envs/tgnn_3.10/bin/python', str(OUT.parent / 'flickr_models_10ep_20260912/compare.py')], check=True)
