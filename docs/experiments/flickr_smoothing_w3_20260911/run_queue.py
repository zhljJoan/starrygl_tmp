...
ROOT = Path('/mnt/data/zlj/starrygl-experiments/flickr_smoothing_w3_20260911')
...
ENV = dict(os.environ, CUDA_VISIBLE_DEVICES='0,1', NCCL_IB_DISABLE='1', OMP_NUM_THREADS='4',
...
def run(arm, epochs, pilot=False):
    ...
    name = 'pilot_gconv_smooth_v2' if pilot else f'gconv_gru_{arm}_s42'
    output = ROOT / 'results' / name
    artifact = OLD / 'prepared_gconv_gru_2rank' if arm == 'exact' else ROOT.parent / 'flickr_window_slots_20260911/prepared_gconv_gru_hot'
    command = [PYTHON, '-m', 'torch.distributed.run', '--standalone', '--nproc_per_node=2',
        ...
        '--output', str(output), '--model', 'gconv_gru', '--policy', 'exact' if arm == 'exact' else 'bounded_stale',
        '--num-full-snapshots', '3', '--access-pipeline', '--epochs', str(epochs), '--eval-every', '5']
    if arm == 'smooth':
        command.append('--compensate')
    record = dict(name=name, command=command, started_at_unix=time.time(), gpus='0,1', status='running')
...
if __name__ == '__main__':
    ...
    run('smooth', 1, pilot=True)
    for arm in ('exact', 'cache', 'smooth'):
        run(arm, 100)
    (ARCHIVE / 'completion.json').write_text(json.dumps(dict(completed_at_unix=time.time(), arms=3, epochs_each=100), indent=2)+'\n')
